...
phase_check(){
  ...
  ( cd "$PIPE" && python3 build_vehicle_base.py --check >/dev/null 2>&1 ); rc=$?
  ...
  ( cd "$PIPE" && python3 check_provenance_doc.py --check >/dev/null 2>&1 ) && ok "check_provenance_doc.py --check" || bad "check_provenance_doc.py --check (CLAUDE.md's provenance census count drifted from platform/data/provenance.json — run: python3 pipeline/check_provenance_doc.py)"
  ...
  ( cd "$PIPE" && python3 build_peer_asset_quality.py --check >/dev/null 2>&1 ); rc=$?
  if [ "$rc" -eq 0 ]; then ok "build_peer_asset_quality.py --check"
  elif [ "$rc" -eq 3 ]; then skip "build_peer_asset_quality.py --check (source-data/set_filings corpus absent — SET filings pull, not data drift)"
  else bad "build_peer_asset_quality.py --check (peer_asset_quality.json drifted from source-data/set_filings NOTES — run: python3 pipeline/build_peer_asset_quality.py)"
  fi
...
INGESTS
  ...
  ( cd "$PIPE" && python3 build_agri_squeeze.py --check >/dev/null 2>&1 ) && ok "build_agri_squeeze.py --check" || bad "build_agri_squeeze.py --check (agri_squeeze.json drifted from crop_stress.json/peer_province.json — run: python3 pipeline/build_agri_squeeze.py)"
  ( cd "$PIPE" && python3 build_collateral_squeeze.py --check >/dev/null 2>&1 ) && ok "build_collateral_squeeze.py --check" || bad "build_collateral_squeeze.py --check (collateral_squeeze.json drifted from collateral_outlook.json/peer_province.json — run: python3 pipeline/build_collateral_squeeze.py)"
