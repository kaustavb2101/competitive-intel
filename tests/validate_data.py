...
# ---------------------------------------------------------------------------
def check_factory_income():
    # Per-province factory-worker income floor (pipeline/build_factory_income.py), projected from
    # the already-MEASURED household_income_by_province.json. Optional file: SKIP-PASS when absent
    # (the builder degrades to an absent-state too).
    hdr("factory_income_by_province.json (optional)")
    if not exists("factory_income_by_province.json"):
        ok("factory_income_by_province.json absent — skipped (optional; run build_factory_income.py)")
        return
    try:
        d = load("factory_income_by_province.json")
    except Exception as e:
        fail("factory_income_by_province.json loads", repr(e))
        return
    ok("factory_income_by_province.json loads")

    meta = d.get("meta")
    if not isinstance(meta, dict) or "generated_by" not in meta or "source" not in meta:
        fail("factory_income meta/provenance present", "meta missing generated_by/source")
    else:
        ok("factory_income meta/provenance present")

    if meta and meta.get("absent"):
        ok("factory_income is an honest ABSENT-state (no source) — skipped value checks")
        return

    national_avg = meta.get("national_avg") if meta else None
    if not is_finite_number(national_avg) or national_avg <= 0:
        fail("factory_income national_avg positive", repr(national_avg))
        return

    provs = d.get("provinces")
    if not isinstance(provs, dict) or not provs:
        fail("factory_income has a 'provinces' dict", "got %s" % type(provs).__name__)
        return
    ok("factory_income provinces dict present (%d)" % len(provs))

    bad = []
    for name, rec in provs.items():
        fi = rec.get("factory_income")
        ratio = rec.get("ratio_to_national")
        if not is_finite_number(fi) or fi <= 0:
            bad.append("%s factory_income=%r not positive" % (name, fi))
            continue
        if not is_finite_number(ratio) or ratio <= 0:
            bad.append("%s ratio_to_national=%r not positive" % (name, ratio))
            continue
        expect = round(fi / national_avg, 3)
        if abs(ratio - expect) > 0.001:
            bad.append("%s ratio_to_national=%s != factory_income/national_avg=%s" % (name, ratio, expect))
    if bad:
        fail("factory_income values sane (positive, ratio=factory_income/national_avg)", first_n(bad, 8))
    else:
        ok("factory_income values sane (measured, ratio recomputed and matches)")


# ---------------------------------------------------------------------------
def check_agri_income():
    # Per-province agriculture-worker income floor (pipeline/build_agri_income.py), projected from
    # the already-MEASURED household_income_by_province.json. Optional file: SKIP-PASS when absent
    # (the builder degrades to an absent-state too). Mirrors check_factory_income() for a different
    # NSO SES occupation column.
    hdr("agri_income_by_province.json (optional)")
    if not exists("agri_income_by_province.json"):
        ok("agri_income_by_province.json absent — skipped (optional; run build_agri_income.py)")
        return
    try:
        d = load("agri_income_by_province.json")
    except Exception as e:
        fail("agri_income_by_province.json loads", repr(e))
        return
    ok("agri_income_by_province.json loads")

    meta = d.get("meta")
    if not isinstance(meta, dict) or "generated_by" not in meta or "source" not in meta:
        fail("agri_income meta/provenance present", "meta missing generated_by/source")
    else:
        ok("agri_income meta/provenance present")

    if meta and meta.get("absent"):
        ok("agri_income is an honest ABSENT-state (no source) — skipped value checks")
        return

    national_avg = meta.get("national_avg") if meta else None
    if not is_finite_number(national_avg) or national_avg <= 0:
        fail("agri_income national_avg positive", repr(national_avg))
        return

    provs = d.get("provinces")
    if not isinstance(provs, dict) or not provs:
        fail("agri_income has a 'provinces' dict", "got %s" % type(provs).__name__)
        return
    ok("agri_income provinces dict present (%d)" % len(provs))

    bad = []
    for name, rec in provs.items():
        ai = rec.get("agri_income")
        ratio = rec.get("ratio_to_national")
        if not is_finite_number(ai) or ai <= 0:
            bad.append("%s agri_income=%r not positive" % (name, ai))
            continue
        if not is_finite_number(ratio) or ratio <= 0:
            bad.append("%s ratio_to_national=%r not positive" % (name, ratio))
            continue
        expect = round(ai / national_avg, 3)
        if abs(ratio - expect) > 0.001:
            bad.append("%s ratio_to_national=%s != agri_income/national_avg=%s" % (name, ratio, expect))
    if bad:
        fail("agri_income values sane (positive, ratio=agri_income/national_avg)", first_n(bad, 8))
    else:
        ok("agri_income values sane (measured, ratio recomputed and matches)")
...
def check_vehicle_flow():
    ...
    # <slug>.json's gov.vehicle_flow. Lives in source-data/ (not platform/data/), so checked via an
    # explicit path like check_heng_branches(). SKIP-passes whole when the source layer is absent.
    hdr("source-data/vehicle_flow_by_province.json (optional)")
    path = os.path.join(REPO, "source-data", "vehicle_flow_by_province.json")
    if not os.path.exists(path):
        ok("vehicle_flow_by_province.json absent — skipped (optional; run build_vehicle_flow.py)")
        return
    try:
        d = json.load(open(path, encoding="utf-8"))
    except Exception as e:
        fail("vehicle_flow_by_province.json loads", repr(e))
        return
    ok("vehicle_flow_by_province.json loads")

    meta = d.get("meta")
    if not isinstance(meta, dict) or not all(k in meta for k in ("generated_by", "source", "formula")):
        fail("vehicle_flow meta/provenance present (generated_by + source + formula)",
             "meta=%r" % (meta,))
    else:
        ok("vehicle_flow meta/provenance present (generated_by + source + formula)")

    provs = d.get("provinces")
    if not isinstance(provs, dict) or not provs:
        fail("vehicle_flow has a non-empty 'provinces' dict", "got %s" % type(provs).__name__)
        return
    ok("vehicle_flow provinces dict present (%d)" % len(provs))

    bad = []
    for name, buckets in provs.items():
        if not isinstance(buckets, dict) or not all(b in buckets for b in ("all", "car", "pickup", "moto")):
            bad.append("%s missing an all/car/pickup/moto bucket" % name)
            continue
    ...
    if bad:
        fail("vehicle_flow per-province values sane (buckets present, counts>=0, rates in [0,1])",
             first_n(bad, 8))
    else:
        ok("vehicle_flow per-province values sane (buckets present, counts>=0, rates in [0,1])")

    # join check: provinces/<slug>.json's gov.vehicle_flow must match this source file exactly
    # (build_province.py does a pure pass-through join — no recomputation).
    try:
        idx = load(os.path.join("provinces", "index.json"))
    except Exception as e:
        fail("provinces/index.json loads (for vehicle_flow join check)", repr(e))
        return
    jbad = []
    n_checked = 0
    for e in idx if isinstance(idx, list) else []:
        ...
        try:
            ...
            continue
        n_checked += 1
        vf = (p.get("gov") or {}).get("vehicle_flow")
        expect = provs.get(th) or {}
        if vf != expect:
            jbad.append("%s gov.vehicle_flow != source-data row for '%s'" % (slug, th))
    if jbad:
        fail("provinces/*.json gov.vehicle_flow join matches source exactly", first_n(jbad, 8))
    else:
        ok("provinces/*.json gov.vehicle_flow join matches source exactly (%d provinces checked)"
           % n_checked)
...
# files by any pipeline script), so nothing previously verified the board's numbers
# actually match the raw pull it claims to summarize — this check closes that gap.
_COMMODITY_BOARD_SOURCE_MAP = {
    "Rice":      ("commodities.json", "rice"),
    "Rubber":    ("commodities.json", "rubber"),
    "Sugar":     ("commodities.json", "sugar"),
    "Palm oil":  ("commodities.json", "palm"),
    "Maize":     ("commodities.json", "maize"),
    "Chicken":   ("commodities_protein.json", "chicken"),
    "Beef":      ("commodities_protein.json", "beef"),
    "Fishmeal":  ("commodities_protein.json", "fishmeal"),
    "Logs":      ("commodities_protein.json", "logs"),
    "Sawnwood":  ("commodities_protein.json", "sawnwood"),
    "Gold":      ("commodities_protein.json", "gold"),
}


def check_commodity_board():
    # source-data/commodity_board.json — the World Bank Pink Sheet board every macro/collateral/
    # crop-stress builder reads (build_macro_exposure.py, build_macro_sensitivity.py,
    # build_collateral_outlook.py, build_crop_stress.py, derive.py's meta.board passthrough).
    # It is committed directly (not regenerated by any --check-gated builder), so this check
    # verifies its per-row 'yoy'/'stale' claims are byte-exact against the two raw World Bank
    # pulls that back it (source-data/commodities.json crops + commodities_protein.json
    # livestock/fisheries/forestry/gold) — catches silent hand-edit drift from the cited source.
    hdr("source-data/commodity_board.json (traceability to raw Pink Sheet pulls)")
    board_path = os.path.join(REPO, "source-data", "commodity_board.json")
    if not os.path.exists(board_path):
        fail("commodity_board.json exists", "missing at %s" % board_path)
        return
    try:
        board = json.load(open(board_path, encoding="utf-8"))
    except Exception as e:
        fail("commodity_board.json loads", repr(e))
        return
    ok("commodity_board.json loads")

    if not isinstance(board, list) or not board:
        fail("commodity_board.json is a non-empty list of rows", "got %s" % type(board).__name__)
        return
    reqk = ("lab", "yoy", "seg", "reg", "note", "cls", "stale")
    bad = [r.get("lab", "?") for r in board if not isinstance(r, dict) or not all(k in r for k in reqk)]
    if bad:
        fail("commodity_board rows carry lab/yoy/seg/reg/note/cls/stale", first_n(bad, 8))
    else:
        ok("commodity_board rows carry lab/yoy/seg/reg/note/cls/stale (%d rows)" % len(board))

    raw_cache = {}
    def _raw(fname):
        if fname not in raw_cache:
            raw_cache[fname] = json.load(open(os.path.join(REPO, "source-data", fname), encoding="utf-8"))
        return raw_cache[fname]

    mismatches, unmapped, checked = [], [], 0
    for row in board:
        if not isinstance(row, dict):
            continue
        lab = row.get("lab")
        mapping = _COMMODITY_BOARD_SOURCE_MAP.get(lab)
        if not mapping:
            unmapped.append(lab)
            continue
        fname, key = mapping
        try:
            src_entry = _raw(fname).get(key)
        except Exception as e:
            mismatches.append("%s: could not load %s (%r)" % (lab, fname, e))
            continue
        if not isinstance(src_entry, dict):
            mismatches.append("%s: %s['%s'] missing/malformed" % (lab, fname, key))
            continue
        checked += 1
        if row.get("yoy") != src_entry.get("yoy"):
            mismatches.append("%s: board yoy=%r != %s['%s'].yoy=%r" %
                               (lab, row.get("yoy"), fname, key, src_entry.get("yoy")))
        if row.get("stale") != src_entry.get("date"):
            mismatches.append("%s: board stale=%r != %s['%s'].date=%r" %
                               (lab, row.get("stale"), fname, key, src_entry.get("date")))

    if unmapped:
        fail("commodity_board every row maps to a known raw-pull source",
             "unmapped labels: %s (add to _COMMODITY_BOARD_SOURCE_MAP if genuinely a new source)"
             % first_n(unmapped, 8))
    else:
        ok("commodity_board every row maps to a known raw-pull source (%d labels)" % len(_COMMODITY_BOARD_SOURCE_MAP))

    if mismatches:
        fail("commodity_board yoy/stale values match the raw Pink Sheet pulls exactly", first_n(mismatches, 8))
    else:
        ok("commodity_board yoy/stale values match the raw Pink Sheet pulls exactly (%d rows checked)" % checked)
