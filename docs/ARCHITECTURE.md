...
│   └── data/  branches.json  meta.json  rayong_province.json  rayong_catchment.json
├── pipeline/                        ← Python (run locally; some needs Thai IP)
│   ├── autox_enrich_loop.py  autox_dgt_ingest.py  regionmap.py
│   ├── save_competitors.py  pull_buildings.py  pull_wide.py  build_platform.py
...
- **rayong-province.html** ← `data/rayong_province.json` (districts geojson + branches + competitors + poi + facts).
...
- `rayong_province.json` = filter `branches_final.json` to `prov contains ระยอง`, join `rayong_districts.geojson`,
  attach `rayong_competitors.json` + Rayong-clipped `osm_layers.json` + the facts block.
- `rayong_catchment.json` = `pull_wide.py` (buildings) → compute floor-area (footprint × levels) →
  filter branches/competitors/POI to the building bbox → emit. `build_platform.py` then assembles the page.
...
- Rayong views use **deck.gl 8.9.35** for true 3D (extruded polygons / buildings), but each is its **own page**
  so only one WebGL context is alive at a time. `build_platform.py` injects a shared `#nav` and a `fetch()`
  loader (with loading + error states) into each Rayong page.
