...
## 2026-07-18 — Intelligence loop: PEER/MARKET — industry saturation vs the collateral base (operators per 100k registered vehicles) — SHIPPED
- **State verified first (not assumed):** gate green on master (`bash tests/run.sh check` → 65/0); `AUTONOMY_PLAN.md` shows **0 open items** (96%, only owner-side in-progress: real loan tape). Deploy-health probe of the master production alias returned **401** = healthy-but-gated (middleware Basic Auth), not a regression. `SERVICE_AUDIT.md` reports the data room healthy, no broken refs. So rather than manufacture churn, I took the highest-value **verifiable deepening** of an existing MEASURED layer.
- **Gap:** the per-province peer board (`#acq`, `peer_province.json`) shows raw AutoX/rival counts and the rivals÷AutoX ratio, but had **no read of how crowded the title-loan industry is relative to the collateral base it lends on**. For a title lender the natural denominator is registered vehicles (what a title loan is secured against) — the missing margin-erosion signal for objective #2. The denominator was already committed and MEASURED (`source-data/vehicles_by_province.json`, DLT stock, 44.3M vehicles, 77-province clean join) but never joined to the peer board.
- **Ship (one improvement, 2 code files + 2 regenerated data files):**
  - `pipeline/build_peer_province.py`: folds in `vehicles_by_province.json` (graceful/degrades to null if absent). Each province gains `vehicles` (MEASURED), `operators_total` = autox+rivals (COMPUTED, a LOWER BOUND — inherits the rival-census undercount), `sat_per_100k` = operators per 100k vehicles (COMPUTED), and `sat_rank` (1 = most saturated). Meta gains `saturation_available` flag, `national_sat_per_100k` (**41.8**), `most_saturated` (**สมุทรปราการ 257.0**), `least_saturated` (ยะลา 10.9), `total_vehicles`, `vehicles_source`, plus a provenance entry + a caveat (denominator is TOTAL registered vehicles, not filtered to title-loan-likely vehicles, so vehicle-rich Bangkok reads low; lower-bound inherited).
  - `platform/app.js` (`drawPeerProvince`): adds a **Sat./100k veh** column (gated on `m.saturation_available` so a pre-fold file degrades) — red ≥100, gold ≥50 — with an operators÷vehicles + rank tooltip, a readout headline naming the most-saturated province vs the national average, and a method bullet spelling out the MEASURED-÷-MEASURED basis and the LOWER-BOUND / not-a-share caveat.
- **Why it matters:** the ratio column answers "how outnumbered is AutoX here"; saturation answers the different, sharper question "is this province over-served by the whole title-loan industry relative to its collateral base" — a margin-pressure read on the footprint we already run. Bangkok's satellites (Samut Prakan, Pathum Thani, Nonthaburi) top the list; the deep south (Yala, Pattani, Narathiwat) sits lowest.
- **Safeguards (all pass):** (a) `bash tests/run.sh check` → **65 passed, 0 failed** (incl. `build_peer_province.py --check` + `build_provenance.py --check` + `node --check app.js`). (b) `build_provenance.py` regenerated + `--check` OK (peer_province.json size drift folded in, 314/314 labelled). (c) no secrets in diff. (d) all inputs MEASURED, ratio COMPUTED, honest caveats — no fabrication. Headless render of `#acq` (Chromium): the column, all 20 per-row sat cells (correct operators÷vehicles math + rank tooltips), readout headline, and method note all verified; no console error, no not-yet-computed state.
- **Deploy-verify:** recorded below after the push.
- **Next recommended intelligence task:** join the same vehicle-collateral denominator into the per-district `rival_density.json` (amphoe-level saturation, the finer grain), or add a `vehicles`-normalised column to the branch-level rival-pressure read — both extend this collateral-base lens to the other two competitive boards.
