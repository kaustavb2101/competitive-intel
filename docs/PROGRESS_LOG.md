...
## 2026-07-23 — Intelligence loop (FOLD): surface the dark farmer-margin layer on Overview (obj #1) — PR
- **Improvement:** `intel(overview): surface the MEASURED-input farmer-margin (OAE cost vs NABC price) layer on the crop-stress section` — `platform/data/crop_margin.json` was a **committed, byte-reproducible, gated** obj-#1 layer (`build_crop_margin.py --check` byte-exact in `tests/run.sh check`) that **nothing in the app ever fetched** — the same "present-in-data-yet-invisible" gap the prior run flagged as the natural next fold (2026-07-20 log: *"crop_margin.json … the natural next two Overview/portfolio-risk folds"*). It is the **repayment-capacity companion** to the crop-stress + drought reads: those name the rainfall/price hazard; this answers whether each crop currently **clears its own OAE production cost** (positive margin = the agri borrower can service debt this season; a thin margin = the repayment edge). Added a lazy loader (`loadCropMargin`) + `renderCropMargin()` + a themed card directly under the district-drought card in the `#overview` crop-stress section: a verdict line (**7 of 7 joined crop rows clear cost; thinnest = rubber at 26% of price, +38% YoY**), an honesty note (**MEASURED** inputs — OAE cost year 2567/68 · NABC live farm-gate as of 2026-07-17 — but the per-rai margin is **DERIVED** across two vintages → *read direction and ranking, not decimals*), and a crops table sorted **thinnest-margin-first** (rubber 26% → maize 35% → cassava 38% → …) with per-row `measured`/`derived` tags mirroring each crop's `cost_method`. Provenance-honest: the layer carries the solid `● measured · OAE cost · NABC price` chip on its inputs while the note flags the derived arithmetic. Null-safe: absent/empty file → the whole card stays `display:none` and the Overview reads exactly as before.
- **No data change:** pure surfacing of an already-committed, already-gated MEASURED-input layer — **zero** pipeline/data edits, so no layer regenerated. `build_provenance.py` re-ran → byte-identical (`crop_margin.json` was already registered).
- **Safeguards (all pass):** (a) `bash tests/run.sh check` → **85 passed, 0 failed** (incl. every `--check` builder byte-exact + `build_provenance.py --check` + `validate_data.py` 446/0 + `node --check` on `app.js`). (b) headless render of `index.html#overview` (1200×2800, swiftshader harness) → **`data-errors="[]"`** (zero uncaught page errors); settled DOM confirms `#crop-margin-wrap` is `display:block`, the verdict reads *"…clears OAE production cost on 7 of 7 joined crop rows … Thinnest: rubber (raw sheet) at 26% of price (price +38% YoY)"*, the note carries the MEASURED/DERIVED caveat, and the table renders 8 rows (header + 7 crops) thinnest-first; the top-of-page PNG was self-reviewed — no layout breakage. (c) no secrets in diff. (d) diff = 2 files (`platform/app.js` loader+render+call, `platform/index.html` container), no stray files.
- **Decision — PR not direct commit:** the change adds a visible Overview card (alters app visuals), so per the loop's ship rule it goes as a PR rather than a straight-to-master commit, despite the green gate + clean DOM verify.
- **Next recommended integration:** `region_debt.json` (BoT regional household-debt backdrop) is the last of the three obj-#1 dark folds the 2026-07-20 run named and follows the identical surface-an-existing-gated-layer pattern. Owner-side unlocks unchanged: real loan tape (flips the 4 SYNTHETIC outputs to measured), `GISTDA_SPHERE_KEY` into a workflow `env:` (40m crop-area), Thai-IP baac/smebank re-pull. (Concurrent note: PR #117 shipped the sibling `province_lfs.json` fold onto Overview the same day; this branch merged master and kept both cards.)
