...
## 2026-07-24 — Intelligence loop (MARKET, merchant segment): surface the last dangling MEASURED layer — DBD new-firm formation as a merchant-DEMAND backdrop on Overview — PR

- **State verified first:** gate green on master baseline (`bash tests/run.sh check` → **87 passed, 0 failed**, incl. `node --check` on every page's inline JS + full determinism gate + 446 data-integrity checks). Production alias healthy (`/` and `/data/meta.json` both **HTTP 200**). `AUTONOMY_PLAN` reads 94% (47 done / 2 in-progress [real loan tape, owner-side] / 1 open [P3 access-protection, already verified live]). Re-audited the never-surfaced committed layers (grep of every basename across `platform/*.html` + `app.js`): after the recent `truck_flow`/`brand_trends` surfaces, only **two** remained consumed by their own builder alone — `dbd_formation.json` and `thaiwater_flood.json`. Skipped the flood layer this run: it is a **live** pull last refreshed 2026-07-11 (13 days stale, no CI refresh workflow) and currently shows **no** province at bank-overflow — a weak, staleness-undermined signal to present as "live." Picked the clean measured one.
- **The gap (a dangling MEASURED layer, MARKET / merchant segment):** `platform/data/dbd_formation.json` — MEASURED DBD (Department of Business Development) monthly new juristic-person (company) registrations, one snapshot month (2569-06 / June 2026), tallied by the registry's own head-office province — was **never surfaced anywhere in the app**. It carries the one borrower segment the Overview macro cluster had no demand read for: the **merchant / small-business owner** (a real AutoX segment). It answers *where the small-business borrower base is regenerating this month*.
- **Framing discipline (no-expansion mandate):** the raw layer is by-province, and a province table topped by Bangkok invites a "go open there" misread — exactly what CLAUDE.md forbids. So the card rolls the 77 provinces up **client-side to AutoX's 5 operating regions** (province→region map built from the branch set's own `d.v`/`d.r`; all 7,972 firms map, 0 unmapped) and carries an **AutoX-branches-per-region** column, so it reads as a demand backdrop **on the footprint we already run**, at a grain that cannot be read as a where-to-open cue. Verdict states outright "Demand context for the merchant segment, not a footprint call"; note states "Makes no open/close recommendation."
- **The analytical read (not a data dump):** new small-business formation is **metro-concentrated — Central & Bangkok takes 54.0% of the month's 7,972 new firms and 57.3% of registered capital**, while AutoX's branch weight is heaviest in **Isan (601 branches), which sees only 12.7% of formation**. The read: the merchant-borrower base is regenerating fastest where AutoX is thinner, and the demand pulse is thinnest in the agri regions where the book is densest — a demand-mix backdrop for the merchant segment, framed as context, not a footprint call.
- **Ship (`platform/index.html` +12 markup, `platform/app.js` +72; no data/provenance change):** a null-safe `#dbdform-wrap` block on `#overview`, placed after `#truckflow-wrap` in the portfolio/segment cluster (`h2.demand` teal accent). `loadDbdFormation()` (promise-cached lazy fetch, mirrors `loadTruckFlow`) + `renderDbdFormation()` read `dbd_formation.json`, build the province→region rollup + AutoX-branch counts in the browser from the committed branch set, lead with the concentration verdict, and render a 5-region table (New firms/mo · share · Reg. capital · share · AutoX branches). Warm-loaded at boot beside `renderTruckFlow`. Tagged **MEASURED · DBD registry**; every number reads the layer's own rows or the branch set. Registered-capital caveat (authorised-not-paid-up, few-large-dominated → read as directional) and the one-month-flow-not-stock caveat carried in-note.
- **No fabrication / no data change:** pure app surfacing of an already-committed MEASURED layer — **zero** pipeline/data edits, so no layer regenerated and provenance untouched (`build_provenance.py` not required; the layer is already provenanced). Absent/empty layer OR absent branch set → the wrap stays `display:none` (nothing fabricated).
- **Safeguards (all pass):** (a) `bash tests/run.sh check` → **87 passed, 0 failed** (no data file touched so all `--check` builders stay byte-exact; `node --check app.js` clean). (b) **headless render** of `index.html#overview` (system Chromium via `tests/lib/render.sh`, 1300×3800) → settled DOM shows `#dbdform-wrap` unhidden (`style=""`), `data-errors="[]"` (0 page/JS errors), the verdict carrying "Central & Bangkok takes 54.0% of the month's 7,972 new firms and 57.3% of registered capital" + "AutoX's branch weight is heaviest in Isan (Northeast) (601 branches), which sees only 12.7%", and the 5-region table matching an independent node/python re-computation against the committed layer exactly (Central&BKK 4,305/54.0%/฿8.73bn/580 → South 740/9.3%/฿1.22bn/250). (c) no secrets in diff. (d) diff = exactly 2 files (`platform/index.html`, `platform/app.js`) + this log entry, additive (+84). Changes a visible card → opened as a **PR** per the ship protocol.
- **Next recommended intelligence task:** the never-surfaced-MEASURED well is now down to `thaiwater_flood.json` alone — which wants a **refresh workflow** (a `.github/workflows/data-thaiwater.yml` re-running `pull_thaiwater_flood.py` on a cron) before it is honest to surface as a "live" flood pulse; that (SERVICE/DEPLOYMENT pillar) is the natural next pick. Owner/key-side unlocks unchanged: real loan tape flips the four portfolio outputs SYNTHETIC→measured; `GISTDA_SPHERE_KEY` into a workflow `env:` for the 40m cropland puller.
