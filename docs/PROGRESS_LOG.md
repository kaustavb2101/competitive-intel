...
## 2026-07-23 — Intelligence loop (MARKET/obj #1): surface the MEASURED farmer-margin layer (farm-gate price vs OAE cost) on Overview — PR

- **State verified first:** synced to latest master, gate green on baseline (`bash tests/run.sh check` → **85 passed, 0 failed**). Re-confirmed the explicit integration backlog is DONE or blocked here: FPO PICO census (#1) wired, per-branch cropland (#2) surfaced, **data.go.th distillation (#3) blocked in CI** (`source-data/datagoth/` cache is gitignored — verified ABSENT — and the aggregator is 403 from CI, so `baac_credit`/`smebank_credit` can't be re-pulled or distilled cloud-side), **GISTDA (#4) blocked** (`GISTDA_SPHERE_KEY` absent from CI env), real loan tape owner-side. So I built the **explicitly-recommended next intelligence task** named in the prior 2026-07-23 entry: surface one of the two dangling MEASURED obj-#1 layers.
- **The gap (a dangling MEASURED fold, obj #1):** `platform/data/crop_margin.json` (`build_crop_margin.py` — OAE production cost, crop year 2567/68, joined against the live NABC farm-gate price per crop) is a committed, provenanced, MEASURED-input layer that **nothing in the app ever fetched** (verified: no reference in any `.js`/`.html`; its own builder is the sole consumer). It is the **measured reality behind the estimated crop-stress proxy** already on Overview: crop-stress flags where prices are *softening* + drought elevated (estimated), while crop_margin answers the harder question — do current farm-gate prices actually **cover the cost of growing** = agri repayment capacity behind the title book (obj #1, the resilient-agri story). Picked over the other dangling layer (`region_debt.json`) because it is more concrete, has cleaner provenance (fewer mixed-confidence rows), and slots directly beside the existing crop cluster.
- **The analytical read (not a data dump):** farm-gate price currently **covers OAE cost on 7 of 7 joined crop rows** — every crop clears its cost today, so agri cash-flow is intact for now. The card leads with the **thinnest cushion** (risk lens): **rubber** at 26% of price (most exposed if prices soften), then maize 35% / cassava 38%, up to **oil palm** 64% (fattest). Prices are broadly rising YoY (cassava +58%, rubber +38%, oil palm +33%), reinforcing the "intact for now" read. Sorted thinnest-first so the least-buffered grower segments read at the top.
- **Ship (`platform/index.html` +10 markup, `platform/app.js` +56; no data/provenance change):** a null-safe `#cm-wrap` block on `#overview`, placed in the obj-#1 crop cluster right after the provincial-labour-stress card. `loadCropMargin()` (promise-cached lazy fetch, mirrors `loadNapprang`/`loadProvinceLfs`) + `renderCropMargin()` read `crop_margin.json` and render a 7-row table (crop · farm-gate ฿/kg · YoY · cost ฿/kg · derived margin ฿/rai · margin % of price, bar-coloured on the theme tokens), sorted by margin % ascending. Absent/empty layer → the wrap stays `display:none` (nothing fabricated).
- **Honesty:** inputs labelled **MEASURED** (OAE cost + NABC farm-gate price), the **margin arithmetic is DERIVED** and the two vintages differ, so the note leads with **"read direction, not decimals"**; cost figures back-computed from OAE ฿/ton × yield carry a `◆` derived marker; omitted crops (sugarcane, durian, longan — no joined pair) are named. Makes **no** open/close/expand recommendation — a risk lens on the footprint we already run. Every number reads from the committed layer's own rows.
- **No fabrication / no data change:** pure app surfacing of an already-committed MEASURED layer — **zero** pipeline/data edits, so no layer regenerated and provenance untouched (`crop_margin.json` already in `provenance.json`; `build_provenance.py` not required).
- **Safeguards (all pass):** (a) `bash tests/run.sh check` → **85 passed, 0 failed** (incl. `node --check app.js` + full determinism gate + 446 data-integrity checks; no data file touched so all `--check` builders stay byte-exact). (b) **headless render** of `index.html#overview` (Chromium/`render.sh`) → settled DOM shows `#cm-wrap` unhidden, `data-errors="[]"` (0 page/JS errors), the note carrying "7 of 7 … read direction, not decimals" and leading with rubber as thinnest cushion, and the table with 7 data rows ordered rubber 26% → oil palm 64%; placement confirmed in the DOM directly after `#lfs-wrap`. (c) no secrets in diff. (d) diff = exactly `platform/app.js` + `platform/index.html` (+ this log entry), no stray files. Changes a visible card → opened as a **PR** per the ship protocol.
- **Next recommended intelligence task:** the last dangling MEASURED obj-#1 layer, `region_debt.json` (BoT regional household debt, region-grain, mixed vintages/confidence — needs a careful honest treatment of its qualitative-vs-numeric rows), onto the Overview macro backdrop. Owner/key-side unlocks unchanged: real loan tape → SYNTHETIC→measured; `GISTDA_SPHERE_KEY` into a workflow `env:`; Thai-IP re-pull + commit of `baac_credit`/`smebank_credit` to unblock the data.go.th credit-penetration distillation (#3).
