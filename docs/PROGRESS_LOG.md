...
## 2026-07-28 — Intelligence loop (INTEGRATION): surface the built-but-invisible PICO district census on `#acq` — district-grain (อำเภอ) competitor clause, obj #2 — PR

- **State verified first:** branched from master `bbb88ca`; baseline `bash tests/run.sh check` → **96 passed, 0 failed** (446/446 data-integrity). Re-audited the scheduled integration backlog against the tree rather than trusting the log: item #1 PICO census is folded nationally AND to district grain (`pico_district.json` exists + gated); #2 `build_branch_cropland.py` is gated + surfaced (23 app.js refs); #3 the un-distilled data.go.th sources (BAAC/SME-bank credit, DBD) stay BLOCKED from CI or owner-side-xlsx-only; #4 `GISTDA_SPHERE_KEY` absent from CI env. The genuine OPEN item was the one the `pico_district` ship itself queued: **the district census was built + gated but never surfaced** — `grep pico_district platform/app.js` → 0 hits (data + provenance only). So I closed that integration gap.
- **The gap (obj #2 — the competitor read stopped at province grain):** the Competition (`#acq`) PICO leaderboard reads sub-scale rival density **per province** and its own method note admitted the limit verbatim: *"the registry carries a province of service, not a coordinate — so this is competitive density by province, not localised within it."* But `pico_district.json` (built 2026-07-27, MEASURED, FPO registry, **1,994 of 2,042 operators = 97.6% address-resolved to 516 of the canonical 928 districts**) already localises that to อำเภอ grain — it was just sitting unread. The exec saw "ร้อยเอ็ด province is dense" but not "**เมืองร้อยเอ็ด** (the provincial-capital district) is the actual hotspot at 40 operators."
- **What shipped (`platform/app.js` only, +29/−4):** a null-safe lazy loader `loadPicoDistrict()` (mirrors the existing `loadPicoCompetitors` pattern) wired into `renderPicoCompetitors` via `Promise.all`, and a MEASURED district-grain clause folded into the existing PICO readout: *"At อำเภอ (district) grain: the sub-scale field clusters hardest in เมืองร้อยเอ็ด, ร้อยเอ็ด (40), เมืองขอนแก่น, ขอนแก่น (28), เมืองพิษณุโลก, พิษณุโลก (27), spread across 516 districts (97.6% of operators address-resolved…; the rest stay in the province total) — the province read localised to the actual district hotspot."* The now-outdated "province-grain, not localised" method note is replaced (when the district layer is present) with the honest resolution provenance; it **falls back to the old province-only note if `pico_district.json` is absent** (graceful degrade, card unchanged). Every rendered number is a pure read of the committed MEASURED layer — no new data, no estimation, the only inference is the file's own documented district string-match; the 2.4% unresolved are stated as staying in the province total, never silently dropped.
- **Verified:** `node --check platform/app.js` clean; clause format sanity-checked against the real `pico_district.json` (top-3 districts + 516/97.6% render correctly). Full gate `bash tests/run.sh check` → **96 passed, 0 failed** (446/446). **Headless render self-review** of `index.html#acq` (system Chromium via `tests/lib/render.sh`, 1200×1800): settled DOM `data-errors="[]"` (zero uncaught JS errors), Leaflet initialised, both the new readout clause AND the updated method note render correctly with the live district data, no layout break.
- **Safeguards (all pass):** (a) gate 0-failed. (b) no secrets in diff (the only "…resolved…" match is prose). (c) diff = exactly `platform/app.js` (+29/−4) + this log — no stray files (render `.cache`/vendor gitignored + auto-cleaned). (d) **no `platform/data` file added or changed** → provenance ledger unchanged (`build_provenance.py --check` byte-exact in the gate) → no regen needed. (e) no fabrication — pure read of the committed MEASURED `pico_district.json`. **App-visual change → PR + headless render self-review (done), not direct-to-master.**
- **Next recommended intelligence task:** the district census is now visible as a headline clause. A natural follow-up: join `pico_district.json` into the National-map amphoe risk lens as a district-grain sub-scale-competitor-density overlay (a bigger UI change → its own PR + render), so the district hotspots are spatial, not just a top-3 list. Higher-value unlocks stay owner/key-side (Thai-IP BAAC/SME-bank credit re-pull; `GISTDA_SPHERE_KEY` 40m cropland; the owner-side `dep-access` Basic-Auth).
