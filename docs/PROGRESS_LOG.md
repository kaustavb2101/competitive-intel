...
## 2026-07-23 — Intelligence loop (MARKET/obj #1): surface the dangling REGIONAL HOUSEHOLD-DEBT backdrop (BoT/NSO SES) on Overview — PR

- **State verified first:** gate green on master baseline (`bash tests/run.sh check` → **86 passed, 0 failed**, incl. `node --check` on every page + full determinism gate + data-integrity checks); production alias healthy (`/`, `/app.js`, `/data/meta.json` all **HTTP 200**). `AUTONOMY_PLAN` reads 94% with the only formal OPEN item a P3 deployment access-protection task already verified live; both in-progress items are the owner-side real-loan-tape unlock. Site-health workflow already targets the correct master production alias (no fix needed).
- **The gap (a dangling MEASURED fold, obj #1 — the explicit "next recommended" from the two prior entries):** `platform/data/region_debt.json` — MEASURED Bank of Thailand regional household-debt read over NSO Socio-Economic Survey data (7 national, 51 regional, 5 province-level series) — was referenced **only** by its own builder (`build_region_debt.py`), never surfaced in the app. It carries the household-debt-stress backdrop behind the *whole* book (not just agri), yet nothing in the platform read it. Verified unreferenced (grep of every basename across `platform/*.html` + `app.js`); its builder is its sole consumer.
- **The analytical read (honest about a messy layer):** the layer mixes vintages (SES 2019/2023, debt-to-GDP Q2 2025) and carries duplicate multi-vintage rows for several region indicators — so the card surfaces **only the clean single-value-per-geo indicators** to avoid picking arbitrarily among dupes. National headline: **60% of Thai households hold < 3 months of financial cushion** (SES 2019), household debt **87% of GDP** (Q2 2025, the freshest figure), avg **฿163,930** per indebted household. The one region indicator with exactly one measured value per region — `unstable_income_share_pct` (SES 2019) — is the sharpest obj-#1 comparable (income instability is a core PD driver): **Northeast 33% · North 32% · South 31% · Central 10%** — income instability runs ~**3.3× higher** in the agri regions than Central, exactly where the agri-PD book sits. Footnote: BoT's 5 named provinces by vulnerable-household share (Surin 32% / Sisaket 30% / Buriram 28% / Satun 20% / Krabi 18%). Copy says **read direction, not decimals** given the mixed vintages, and states BoT publishes no routine province table so region is the honest grain.
- **Ship (`platform/index.html` +11 markup, `platform/app.js` +66; no data/provenance change):** a null-safe `#regiondebt-wrap` block on `#overview`, placed right after the provincial-labour-stress card (keeping the income-side reads together). `loadRegionDebt()` (promise-cached lazy fetch, mirrors `loadProvinceLfs`) + `renderRegionDebt()` read `region_debt.json`, compute the agri-vs-Central instability ratio in-browser from the layer's own rows, and render a 4-region table (coloured on the agri/gold theme tokens) plus the province footnote. Tagged **MEASURED · Bank of Thailand / NSO SES**. Absent/empty layer or missing the clean indicators → the wrap stays `display:none` (nothing fabricated). Warm-loaded at boot beside `loadProvinceLfs`.
- **No fabrication / no data change:** pure app surfacing of an already-committed MEASURED layer — **zero** pipeline/data edits, so no layer regenerated and provenance untouched (`build_provenance.py` not required; the layer is already provenanced). Every number reads the layer's own rows and inherits its vintage caveats; makes **no** open/close/expand recommendation — a risk lens on the footprint we already run.
- **Safeguards (all pass):** (a) `bash tests/run.sh check` → **86 passed, 0 failed** (no data file touched so all `--check` builders stay byte-exact). (b) **headless render** of `index.html#overview` (pre-installed Chromium, `--dump-dom` over a local http server, 1300-wide) → settled DOM shows `#regiondebt-wrap` unhidden, note reading "60% ... < 3 months of financial cushion (SES 2019); ... 87% of GDP (Q2 2025) ... ~3.3× the Central rate (Northeast 33% vs Central 10%)", the 4-region table (Northeast 33 → Central 10), and the 5-province footnote (Surin 32% first). (c) no secrets in diff. (d) diff = exactly 2 files (`platform/index.html`, `platform/app.js`), additive, provenance/no-fabrication intact. Changes a visible card → opened as a **PR** per the ship protocol.
- **Next recommended intelligence task:** with all three 2026-07-20 PDF-ingest-wave MEASURED layers (`province_lfs`, `crop_margin`, `region_debt`) now surfaced, the natural follow-up is a **cross-layer Overview coherence pass** — the income-side cluster (crop-stress → farmer margin → labour stress → household debt) has grown to four adjacent cards; a single lead sentence tying them into one obj-#1 verdict would sharpen the exec read. Owner-side unlocks unchanged (real loan tape flips the four SYNTHETIC portfolio outputs → measured).
