...
| `tmli/` (`nso-ses-debt-2566.json`, `nso-ses-income-2566.json`, `nso-lfs-provincial-summary.json`, `household-debt.js`, `provinces.js`) | MEASURED | NSO SES 2566 (household debt + income) + NSO LFS provincial summary, vendored via `pipeline/ingest_tmli.py` (from `kaustavb2101/watcher`). Provenance recorded in `source-data/tmli/PROVENANCE.md`. | Drives `household_risk_by_province.json` (and thereby the `branch_risk`/`province_risk` composites). |
