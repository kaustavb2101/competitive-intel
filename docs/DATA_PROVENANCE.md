...
**2026-07-12 partial backfill:** the two-objectives-relevant set flagged by the 2026-07-12 audit follow-up
(`branch_peers.json`, `branch_density.json`, `exit_whitespace.json`, `expansion_plan.json`) now has rows
in §1 below — all four already carried strong in-file `meta` (confirmed by reading each file directly),
this closes the "curated tour is incomplete" doc gap for the highest-value four, not a provenance gap
(the gate + `provenance.json` already covered them). Still not backfilled here (lower-priority for the two
standing objectives, but real and gated): `agri_income_by_province.json`, `fuel_prices.json`,
`thaiwater_*.json`, and ~30 others — see `docs/IMPROVEMENT_BACKLOG.md`.
...
| `branch_peers.json` | ESTIMATED composite over MEASURED features (labelled) | `pipeline/build_branch_peers.py` — objective #1 peer-twin outlier benchmark. Each branch's 15 nearest twins are matched on MEASURED market-fingerprint features (OSM 10km POI mix, DIW factories/workers, HDX rainfall, own building density, NSO province-DTI weighted 3x), ≥25km away; `dev`/`rz` compare the branch's ESTIMATED `composite_risk` (`branch_risk.json`) against the twin median. Index-aligned to `branches.json`. | YES `meta.label` (states ESTIMATED PEER BENCHMARK, "twins matched on MEASURED features only … NOT a measured default rate") + `meta.method` + `meta.features`/`meta.params` + `branches_fingerprint` | Honest: the matching features are measured, the thing being compared (risk) is explicitly estimated — surfaced as an audit-first list on `#trend`, not a verdict. |
| `branch_density.json` | MEASURED | `pipeline/build_branch_density.py` — projects the already-committed `source-data/perimeter_counts.json` (Overture building centroids from the 77-province catchment pulls, binned into a 0.1° grid, counted within 10km of each branch). Index-aligned to `branches.json`. | YES `meta.label` (states MEASURED) + `meta.provenance` + `meta.caveats` (explicit zero-means-capped-file caveat + a manually-verified fingerprint-continuity note since the source predates `branches_fingerprint`) + `branches_fingerprint` | Honest: a zero count is flagged as "capped file has none," not "area is empty" — degrades to an honest ABSENT state on a length mismatch rather than guessing a projection (see CLAUDE.md). |
| `exit_whitespace.json` | ESTIMATED PROXY (labelled) | `pipeline/build_exit_whitespace.py` — objective #2 regulatory-tailwind cue. Infers where AutoX could capture share if sub-scale title-loan operators exit under the Q1-2026 BoT registration deadline, from big-4 competitor scarcity (`competitors_census.json` PIP) × our own demand/white-space (`amphoe.json`). | YES `meta.label` (states ESTIMATED PROXY, spells out what it is NOT) + `meta.regulatory_citation` (BoT notification 25680030 + 2 source URLs) + `meta.honesty_caveat` (explicit: the exiting sub-scale operators are NOT censused; only the surviving big-4 are) | Exceptionally honest for an inference-over-inference layer: the file's own `meta` states outright that a true rival-fragility index needs a sub-scale-operator census (a blocked Thai-IP registry pull) that doesn't exist yet. |
| `expansion_plan.json` | ESTIMATED (sequenced plan over MEASURED demand inputs) | `pipeline/build_expansion_plan.py` — objective #2 Road-to-3,000 plan. Greedy divisor (D'Hondt) allocation of the ~985 net-new branches one at a time over risk-adjusted district demand (`amphoe.json`, MEASURED demand legs + ESTIMATED `risk_proxy`), with 15km neighbor-cannibalization damping. | YES `meta.provenance` (states "ESTIMATED planning sequence over MEASURED inputs … NOT a committed branch plan; confirm every site with a local survey") + `meta.method`/`meta.formulas`/`meta.params` | Honest: explicitly a planning ORDER, not a site commitment — the caveat that every site needs a local survey is in the file itself, not just this doc. |
