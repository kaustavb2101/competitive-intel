...
|---|---|---|---|---|---|
| **Real loan tape** | vintage 90+ aging, branch ROI, PD calibration — **direct portfolio risk (#1)** | (core banking export, no host) | — | No open source exists; the four portfolio-risk outputs ship **SYNTHETIC** until a real export lands | Kaustav exports the two no-PII files per `pipeline/loan_tape_schema.md`, drops them in `source-data/`, runs `ingest_loan_tape.py --real`. This is the **single highest-leverage unlock** left. |
