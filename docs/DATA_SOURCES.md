...
> Refreshed 2026-08-23 to the newly-published 2026M07 Pink Sheet (the loop's own cron had left the
> board a month stale at 2026M06); this section previously described the stale Dec-2025 vintage after
> the underlying `source-data/commodit*.json` had already moved on — keep this block in sync with
> `platform/data/meta.json`'s `updated` field whenever the loop re-pulls.
- **Crops mostly UP, but rubber/palm eased from June:** rice +19.1%, rubber +24.7%, palm +12.8%;
  sugar −8.1% (still down, less so); maize +11.1% (firmed from ~flat).
- **Protein/forestry mixed:** beef +8.8%, lamb +19.9%, fishmeal +22.2%; chicken −2.8% and sawnwood
  −1.0% (soft); logs −2.2% (soft). **Gold +21.9%** (matters to a title/gold-collateral
