...
- ux-msheet-handle-keyboard — ✅ FIXED 2026-07-23 — the mobile branch-detail bottom sheet's close handle (`#msheet-handle` in index.html) is `role="button" tabindex="0" aria-label="Close branch detail"`, so it advertises itself as a keyboard-operable button — but `wireBranchSheet()` in app.js only wired a `click` listener, and a `<div role="button">` (unlike a native `<button>`) does NOT fire click on Enter/Space. A keyboard/switch user could focus the visible "Close" control but not activate it (only the global Escape closed the sheet — no visible affordance). Added a `keydown` handler on the handle for Enter/Space (`preventDefault` + `closeBranchSheet()`), matching WCAG 2.1.1 (Keyboard). No pointer-user or visual change; node --check clean.

## Open backlog (discovered by the loop)
- REPO-STATE (not a UX finding, flagged for the owner) — the deployed `master` (origin/master) has **diverged** from a local, unpushed merge commit that carries several already-merged PRs — #115 (chip-aria-pressed), #117 (province-LFS), #119 (mobile-modal focus-management), #120 (farmer-margin). Those fixes are therefore NOT on production. Needs a manual reconcile (merge the local integration line into origin/master, or re-open the PRs against it). Out of scope for a surgical UX run — surfacing so it isn't lost.
