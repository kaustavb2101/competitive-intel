...
- ux-map-overlap-residual (NEW, minor) — between ~761–1050px the lens pills can still wrap to 2 rows while the zoom's 56px top-left offset only clears one row, so the zoom can crowd the 2nd pill row on small laptops / large foldables. Milder than the ≤760px case (map is wide, pills less cramped) and out of the "mobile" finding scope; a fuller fix would relocate the zoom whenever the pill strip wraps (not width-detectable in CSS) or raise the offset — revisit if flagged.
