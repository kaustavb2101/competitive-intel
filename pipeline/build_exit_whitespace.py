...
A true quantitative "rival fragility" index needs a SUB-SCALE-OPERATOR CENSUS
(small/independent title lenders), which is a BLOCKED desktop / Thai-IP pull
(data.go.th / dataforthai company registry). Until that lands, this layer is an
inferred opportunity surface, clearly labelled ESTIMATED everywhere it surfaces.
...
def build():
    ...
    meta = {
        ...
        "honesty_caveat": "The operators that will EXIT are the sub-scale / informal lenders we do "
                          "NOT census. competitors_national.json is a best-effort census of the FOUR "
                          "BIG, COMPLIANT brands (Heng, Muangthai, Tidlor, Srisawad) — the incumbents "
                          "who will SURVIVE. We therefore cannot directly measure exit; we infer the "
                          "opportunity surface from big-4 absence x demand. A true rival-fragility "
                          "index needs a sub-scale-operator census (a BLOCKED desktop / Thai-IP "
                          "company-registry pull). Treat exit_capture_score as ESTIMATED.",
