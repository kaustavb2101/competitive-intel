#!/usr/bin/env python3
"""
build_sme_income.py — per-province SME-owner income floor (portfolio risk, objective #1)

Network-free, deterministic. Mirrors build_factory_income.py / build_agri_income.py's pattern for
a third NSO SES occupation column: SMEOwners. Merchant/collateral lending doesn't yet have a
MEASURED income-floor context card the way manufacturing-base (factory_income_by_province.json)
and agri-relevant (agri_income_by_province.json) branches do — this closes that gap with the same
recipe: which provinces already sit below the national SME-owner income floor, a static context
read independent of any scenario lever.

Input (MEASURED, NSO SES 2566 via the TMLI bridge, ingest_tmli.py):
  source-data/household_income_by_province.json
    provinces: { <Thai province name>: {SMEOwners, ...} }  (THB/month)

Output: platform/data/sme_income_by_province.json
  national_avg          MEASURED — unweighted mean of SMEOwners income across all provinces with a value.
  provinces[<name>]:
    sme_income           MEASURED THB/month (NSO SES 2566).
    ratio_to_national     sme_income / national_avg, rounded 3dp — <1 means below the national floor.
...
Run:
  python3 build_sme_income.py            # write platform/data/sme_income_by_province.json
  python3 build_sme_income.py --check    # re-run, byte-compare against the committed file
"""
import argparse
import json
import os
...
OUT = os.path.join(ROOT, "platform", "data", "sme_income_by_province.json")
...
def build():
    ...
    if src is None:
        meta = {
            "title": "SME-owner income floor by province (portfolio risk, objective #1)",
            "generated_by": "pipeline/build_sme_income.py",
            "deterministic": True,
            "network_free": True,
            "absent": True,
    ...
    meta = {
        ...
        "provenance": "MEASURED. sme_income is the NSO SES 2566 SMEOwners-occupation monthly "
        ...
        "fields": {
            "sme_income": "MEASURED — NSO SES 2566 average monthly SME-owner-occupation income, THB.",
            "ratio_to_national": "MEASURED (derived ratio) — sme_income / national_avg, 3dp. "
                                 "<1.0 means this province's SME-owner income floor sits below "
                                 "the national average.",
        },
        "formula": {
            "national_avg": "round(sum(province SMEOwners values) / n_provinces)",
            "ratio_to_national": "round(sme_income / national_avg, 3)",
        },
        ...
        "caveats": [
            ...
            "book — it reads structural income-floor risk for SME-owner households in that "
            "province (a proxy for merchant-lending demand), not realized PD.",
            "Unweighted across provinces (not population-weighted).",
            "SMEOwners is a distinct NSO SES occupation category from the app's merchant_demand "
            "segment score (which is an Overture establishment-density proxy) — this is a "
            "static income-floor context on business-owner households, not the segment score itself.",
        ],
    }
